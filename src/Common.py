...
def _get_emsdk_include_paths():
  ...
  # On Linux (Docker), prefer system GCC libstdc++ headers over emsdk libc++.
  # libclang 18 cannot parse emsdk's LLVM 21 libc++ headers, causing
  # occ::handle<T> to silently degrade to int.
  if platform.system() == "Linux":
    gcc_base = "/usr/include/c++/"
    if os.path.isdir(gcc_base):
      versions = sorted(next(os.walk(gcc_base))[1], reverse=True)
      if versions:
        gcc_ver = versions[0]
        paths = [
          f"/usr/include/c++/{gcc_ver}",
          f"/usr/include/c++/{gcc_ver}/x86_64-linux-gnu",
          f"/usr/include/x86_64-linux-gnu",
          "/usr/include",
        ]
